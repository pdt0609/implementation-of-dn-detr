...
# COCO area thresholds (in square pixels)
SMALL_THRESHOLD = 32 ** 2   # 1024

# Function to filter and return original tensor for small areas only
def classify_area(boxes_tensor, areas_tensor):
    # Identify small areas (< 1024)
    small_mask = areas_tensor < SMALL_THRESHOLD
    
    # Filter the original boxes tensor to keep only small boxes
    small_boxes = boxes_tensor[small_mask]
...
def prepare_for_dn(dn_args, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    if training:
        ...
        ###############################################
        known_bboxs = boxes.repeat(scalar-1, 1)
        known_bboxs_small = boxes.clone()
        ...
        IMG_WIDTH = 640
        IMG_HEIGHT = 640
        widths = known_bboxs_small[:, 2] * IMG_WIDTH   # width in pixels
        heights = known_bboxs_small[:, 3] * IMG_HEIGHT  # height in pixels
        areas = widths * heights

        small_boxes = classify_area(known_bboxs_small, areas)
        n = boxes.shape[0]  # Extract n (number of rows in tensor2)
        
        if small_boxes.shape[0] == 0:
            # If no small boxes, return a zero-filled tensor of shape [n, 4]
            expanded_small_boxes = boxes.clone()
            # print("not exist small boxes")
        else:
            # Repeat small_boxes to at least n rows and slice to exactly n
            expanded_small_boxes = small_boxes.repeat((n // small_boxes.shape[0]) + 1, 1)[:n] 

        known_bboxs = torch.cat([known_bboxs, expanded_small_boxes], dim=0)
        ...
        # if box_noise_scale > 0:
        #     diff = torch.zeros_like(expanded_small_boxes)
        #     diff[:, :2] = expanded_small_boxes[:, 2:] / 2
        #     diff[:, 2:] = expanded_small_boxes[:, 2:]
        #     expanded_small_boxes += torch.mul((torch.rand_like(expanded_small_boxes) * 2 - 1.0),
        #                                    diff).cuda() * box_noise_scale
        #     expanded_small_boxes = expanded_small_boxes.clamp(min=0.0, max=1.0)

        if box_noise_scale > 0:
            for bbox_tensor in [known_bbox_expand, expanded_small_boxes]:
                diff = torch.zeros_like(bbox_tensor)
                diff[:, :2] = bbox_tensor[:, 2:] / 2
                diff[:, 2:] = bbox_tensor[:, 2:]
                bbox_tensor += torch.mul((torch.rand_like(bbox_tensor) * 2 - 1.0), diff) * box_noise_scale
                bbox_tensor.clamp_(min=0.0, max=1.0)

        known_bbox_expand = torch.cat([known_bbox_expand, expanded_small_boxes], dim=0)
        # print("-------------------known_bbox_expand",known_bbox_expand.shape)
